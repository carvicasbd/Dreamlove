...
public class MainActivity extends Activity {
    private static final String ADMIN = "https://anuncioslimpieza.es/admin?seccion=importador&modo=ofertas";
    ...
    @Override protected void onCreate(Bundle b) {
        super.onCreate(b); prefs=getSharedPreferences("al_ofertas",MODE_PRIVATE); script=asset("extract.js"); buildUi(); configureWeb(); restore(); render("Preparado para importar ofertas desde Android.");
    }
    ...
    private void buildUi(){
        ...
        p.addView(tv("ANUNCIOS LIMPIEZA · ANDROID",11,Color.rgb(25,117,82),true));p.addView(tv("Importador premium de ofertas",22,Color.rgb(17,24,21),true));TextView intro=tv("App Android nativa · sin Chrome, sin Lemur y sin extensiones. El navegador integrado queda visible para resolver manualmente cualquier CAPTCHA.",13,Color.rgb(80,95,88),false);intro.setPadding(0,dp(3),0,dp(6));p.addView(intro);
        token=input("Token del importador","",true);p.addView(token,new LinearLayout.LayoutParams(-1,-2));nSources=input("Fuentes","10",false);nPages=input("Páginas/fuente","2",false);nMax=input("Máx. ofertas","100",false);nDelay=input("Pausa ms","4500",false);p.addView(row(nSources,nPages));p.addView(row(nMax,nDelay));
    ...
    private void render(String m){status.setText("Fuentes "+Math.min(si+1,Math.max(1,sources.size()))+"/"+sources.size()+" · páginas "+pages+"\nOfertas "+demands+" · guardadas "+stored+" · publicadas "+published+" · fotos "+photos+(staged>0?" · foto.pend "+staged:"")+(pendingPhone>0?" · pend.tel "+pendingPhone:"")+(pendingPhoto>0?" · pend.foto "+pendingPhoto:"")+" · omitidas "+skipped+" · errores "+errors+(captchas>0?" · captchas "+captchas:"")+(photoErrors>0?" · err.foto "+photoErrors:"")+"\n"+m);}
    ...
    private final class Js {@JavascriptInterface public void onExtract(String j){main.post(()->{if(running){phase.setText("Procesando ofertas…");process(j);}});}@JavascriptInterface public void onCaptcha(){main.post(MainActivity.this::captcha);}@JavascriptInterface public void onJsError(String m){main.post(()->{errors++;pages++;render("Error de lectura: "+m);pi++;main.postDelayed(MainActivity.this::load,Math.max(3000,delay));});}}
}
