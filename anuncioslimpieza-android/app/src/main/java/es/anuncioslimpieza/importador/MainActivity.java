...
public class MainActivity extends Activity {
    ...
    private WebView web; private EditText token, nSources, nPages, nMax, nDelay; private Spinner province;
    private TextView phase, status; private Button start, stop; private SharedPreferences prefs;
    private BridgeClient client; private String script = "", userAgent = "", selectedProvinceSlug = "", selectedProvinceLabel = "Todas las provincias";
    ...
    private void buildUi(){
        ...
        province=new Spinner(this);ArrayAdapter<String> provinceAdapter=new ArrayAdapter<>(this,android.R.layout.simple_spinner_item,PROVINCE_LABELS);provinceAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);province.setAdapter(provinceAdapter);province.setPadding(dp(8),0,dp(8),0);province.setBackgroundColor(Color.rgb(247,249,248));p.addView(province,new LinearLayout.LayoutParams(-1,dp(48)));
        nSources=input("Fuentes","10",false);nPages=input("Páginas/fuente","2",false);nMax=input("Máx. perfiles","100",false);nDelay=input("Pausa ms","4500",false);p.addView(row(nSources,nPages));p.addView(row(nMax,nDelay));
    ...
    private void restore(){token.setText(prefs.getString("token",""));nSources.setText(String.valueOf(prefs.getInt("sources",10)));nPages.setText(String.valueOf(prefs.getInt("pages",2)));nMax.setText(String.valueOf(prefs.getInt("max",100)));nDelay.setText(String.valueOf(prefs.getInt("delay",4500)));String saved=prefs.getString("province_slug","");int pos=0;for(int i=0;i<PROVINCE_SLUGS.length;i++){if(PROVINCE_SLUGS[i].equals(saved)){pos=i;break;}}province.setSelection(pos);selectedProvinceSlug=PROVINCE_SLUGS[pos];selectedProvinceLabel=PROVINCE_LABELS[pos];}
    ...
    private void startImport(){
        if(running)return;String t=token.getText().toString().trim();if(t.length()<32){toast("Pega el token del Importador Premium de Demandas.");return;}
        int provincePos=Math.max(0,province.getSelectedItemPosition());selectedProvinceSlug=PROVINCE_SLUGS[Math.min(provincePos,PROVINCE_SLUGS.length-1)];selectedProvinceLabel=PROVINCE_LABELS[Math.min(provincePos,PROVINCE_LABELS.length-1)];
        maxSources=num(nSources,10,1,150);pagesPerSource=num(nPages,2,1,10);maxAds=num(nMax,100,1,1000);delay=num(nDelay,4500,3000,30000);prefs.edit().putString("token",t).putString("province_slug",selectedProvinceSlug).putInt("sources",maxSources).putInt("pages",pagesPerSource).putInt("max",maxAds).putInt("delay",delay).apply();client=new BridgeClient(t,userAgent);reset();running=true;getWindow().addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);start.setEnabled(false);stop.setEnabled(true);render("Conectando con anuncioslimpieza.es · "+selectedProvinceLabel+"…");
        io.execute(()->{try{JSONObject r=client.post(new JSONObject().put("action","sources").put("kind","demand"));JSONArray a=r.optJSONArray("sources");if(a==null||a.length()==0)throw new Exception("No hay fuentes configuradas.");for(int i=0;i<a.length()&&sources.size()<maxSources;i++){String u=a.optString(i,"");if(!BridgeClient.isMilanuncios(u))continue;if(selectedProvinceSlug.isEmpty()||matchesProvince(u,selectedProvinceSlug))sources.add(u);}if(!selectedProvinceSlug.isEmpty()&&sources.isEmpty()){String fallback="https://www.milanuncios.com/servicio-domestico-en-"+selectedProvinceSlug+"/";if(BridgeClient.isMilanuncios(fallback))sources.add(fallback);}if(sources.isEmpty())throw new Exception("No hay fuentes válidas para "+selectedProvinceLabel+".");main.post(this::load);}catch(Exception e){main.post(()->fail(e.getMessage()));}});
    }
    private boolean matchesProvince(String raw,String slug){try{String p=Uri.parse(raw).getPath();if(p==null)return false;p=p.toLowerCase(Locale.ROOT);String marker="servicio-domestico-en-"+slug.toLowerCase(Locale.ROOT);return p.contains(marker);}catch(Exception e){return false;}}
